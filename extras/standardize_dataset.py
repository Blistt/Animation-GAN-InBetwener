...
if __name__ == '__main__':
    ...
    if shape is None:
        standard_shape = get_dataset_shapes(data_dir)
        reshape_dataset(data_dir, standard_shape)

    elif isinstance(shape, tuple):
        reshape_dataset(data_dir, shape) 
    else:
        ("Invalid shape. Provide a valid shape in the form (# of rows, # of cols)")

    print('------------------AFTER STANDARDIZATION------------------')
    get_dataset_shapes(data_dir[:-1] + '_standardized/')