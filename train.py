...
from utils.utils import weights_init, visualize_batch
...
def train(tra_dataset, gen, disc, gen_opt, disc_opt, adv_l, adv_lambda, l1=nn.L1Loss(), l2=None, l3=None, 
          lamb1=100, lamb2=100, lamb3=100, n_epochs=10, batch_size=12, device='cuda:1', display_step=20,
    ...
    for epoch in range(n_epochs):
        ...
        if test_dataset is not None:
            ...
            with torch.no_grad():
                test_gen_loss, test_disc_loss = test(test_dataset, gen, disc, adv_l, adv_lambda, l1=l1, l2=l2, l3=l3,
        ...
        if epoch % display_step == 0:
            ...
            if my_dataset is not None:
                ...
                with torch.no_grad():
                    unused_loss = test(my_dataset, gen, disc, adv_l, adv_lambda, l1=l1, l2=l2, l3=l3, lamb1=lamb1, lamb2=lamb2, lamb3=lamb3, 
                                       batch_size=batch_size, device=device, experiment_dir=experiment_dir)
        ...
        print(f"Epoch {epoch}: Training Gen loss: {tr_gen_losses[-1]} Training Disc loss: {tr_disc_losses[-1]} \
              Testing Gen loss: {test_gen_losses[-1]} Testing Disc loss: {test_disc_losses[-1]}")
        
        # Keeps a log of the training and testing losses
        with open(experiment_dir + 'training_log.txt', 'a') as f:
            print(f"Epoch {epoch}: Training Gen loss: {tr_gen_losses[-1]} Training Disc loss: {tr_disc_losses[-1]} \
                  Testing Gen loss: {test_gen_losses[-1]} Testing Disc loss: {test_disc_losses[-1]}", file=f)
...
if __name__ == '__main__':
    ...
    device = 'cuda:1'                   # Device (cuda or cpu)
    ...
    display_step = 1                    # How often to display/visualize the images
    ...
    # training_data_dir = '/data/farriaga/atd_12k/Line_Art/train_10k/'
    ...
    # training_data_dir = '/data/farriaga/atd_12k/Line_Art/test_2k_original/'
    ...
    train(train_dataset, gen, disc, gen_opt, disc_opt, adv_l, adv_lambda, l1=recon_l, l2=gdl_l, l3=ms_ssim_l, lamb1=recon_lambda,
          lamb2=gdl_lambda, lamb3=ms_ssim_lambda, n_epochs=10, batch_size=6, device=device, display_step=display_step,
