...
class GeneratorLight(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels):
        ...
        self.down_sample_blocks = nn.Sequential(
            ...
            nn.Conv2d(hidden_channels, hidden_channels * 2, kernel_size=3, stride=1, padding=1, bias=False),  # size
            ...
            nn.LeakyReLU(0.02, inplace=True),
            nn.Conv2d(hidden_channels * 2, hidden_channels * 2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.Conv2d(hidden_channels * 4, hidden_channels * 4, kernel_size=filter_size, stride=stride_size, padding=1, bias=False),  # size/2
            nn.BatchNorm2d(hidden_channels * 4),
            nn.LeakyReLU(0.02, inplace=True),
            nn.Conv2d(hidden_channels * 4, hidden_channels * 8, kernel_size=3, stride=1, padding=1, bias=False),
            nn.Conv2d(hidden_channels * 8, hidden_channels * 8, kernel_size=filter_size, stride=stride_size, padding=1, bias=False),  # size/2
            nn.BatchNorm2d(hidden_channels * 8),
            nn.LeakyReLU(0.02, inplace=True)
            )
        
        self.up_sample_block = nn.Sequential(
            nn.ConvTranspose2d(hidden_channels * 8, hidden_channels * 4, kernel_size=filter_size, stride=stride_size, padding=1, bias=False),
            ...
            nn.LeakyReLU(0.02, inplace=True),
            nn.ConvTranspose2d(hidden_channels, input_channels, kernel_size=3, stride=1, padding=1, bias=False),  # size*2
            nn.Sigmoid()
            )
