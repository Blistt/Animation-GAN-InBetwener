...
class DiscriminatorFullSpectral(nn.Module):

    def __init__(self, input_channels, hidden_channels):
        super(DiscriminatorFullSpectral, self).__init__()
        ...
        self.conv_blocks = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(0.25),
            nn_utils.spectral_norm(nn.Conv2d(hidden_channels * 16, hidden_channels * 8, kernel_size=4, stride=2, padding=1, bias=False)),
            nn.BatchNorm2d(hidden_channels * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(0.25),
            nn_utils.spectral_norm(nn.Conv2d(hidden_channels * 8, hidden_channels * 8, kernel_size=4, stride=2, padding=1, bias=False)),
            nn.BatchNorm2d(hidden_channels * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(0.25),
            nn_utils.spectral_norm(nn.Conv2d(hidden_channels * 8, 1, kernel_size=4, stride=1, padding=0,bias=False))
        )
